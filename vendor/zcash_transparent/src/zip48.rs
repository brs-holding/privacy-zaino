...
/// Returns the BIP 32 extended-public-key prefix for `params`, or `None` if this crate
/// defines no ZIP 48 extended-key serialization for that network.
fn pub_prefix<P: consensus::Parameters>(params: &P) -> Option<Prefix> {
    match params.network_type() {
        consensus::NetworkType::Main => Some(Prefix::XPUB),
        consensus::NetworkType::Test => Some(Prefix::TPUB),
        consensus::NetworkType::Regtest => Some(Prefix::TPUB),
        // The SWARM production network is explicitly NOT given the upstream mainnet
        // `xpub` prefix and is explicitly NOT folded into the test `tpub` prefix: no
        // ZIP 48 extended-key serialization exists for it yet. Production schedule and
        // derivation constants are admitted by P1c.
        consensus::NetworkType::SwarmMain => None,
    }
}
...
impl AccountPubKey {
    ...
    pub fn parse_key_info_expression<P: consensus::Parameters>(
        ...
        match s.parse::<KeyExpression>().ok()?.into_parts() {
            (Some(origin), descriptor::Key::Xpub { prefix, key, child })
                if Some(prefix) == pub_prefix(params) && child.is_empty() =>
            {
    ...
    pub fn key_info_expression<P: consensus::Parameters>(&self, params: &P) -> String {
        self.key_expression_inner(
            pub_prefix(params).expect("ZIP 48 KEY_INFO expressions are undefined for this network"),
            vec![],
        )
        .to_string()
    }
