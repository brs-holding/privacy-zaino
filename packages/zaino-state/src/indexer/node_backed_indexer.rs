    ...
    for NodeBackedIndexerServiceSubscriber<Source>
{
    ...
    async fn get_lightd_info(&self) -> Result<LightdInfo, Self::Error> {
        ...
        Ok(LightdInfo {
            ...
            taddr_support: true,
            // Zebra labels the public testnet, regtest and a configured
            // testnet alike as "test" on its own RPC. Light clients need the
            // configured network kind to select the correct address prefixes
            // and upgrade schedule, so the label comes from the kind.
            chain_name: self.config.network.lightwallet_chain_name().to_string(),
